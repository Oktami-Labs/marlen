...
- `apps/web`: Vite/React SPA and the API's only client
- `apps/desktop`: shipping Electron shell embedding server and web, with GitHub release updates
- `apps/desktop-tauri`: preview Tauri shell; it uses the dev server and is not
  in the release workflow
- `apps/e2e`: Playwright against an isolated real server, SQLite database, and built SPA
- `packages/shared`: request/response and card types shared by server and web
...
- `pnpm dev`: server on :3001 and web on :5173 with reload
- Tauri preview: run `pnpm dev:server`, then `pnpm dev:tauri` in another terminal
- `pnpm check`: lint, conventions, typecheck, and tests; must pass before a change is done
...
- End-to-end verification: the `verify` skill (`.claude/skills/verify`) boots an
  isolated server and headless UI
- A second Claude session may edit this working tree. Re-check `git status` and
  mtimes before bulk edits.
...
- **Tests are few and behavioral.** Test through stable seams: a real Fastify
  ...
  fixtures. Add tests only where a regression would actually hurt. Never write
...
- **Server layout:** `src/` top level is role-shaped (agent, core, db, email,
  ...
  `src/services/`. New rules land there as a file, or as a folder when they
  grow, never as new top-level folders.

## Direction

Build new code in the target shape:
...
- The model transcript is durable: messages, tool calls, and compaction
  summaries persist and rebuild the model history. Don't reconstruct agent
  state by parsing prose or scraping cards.
