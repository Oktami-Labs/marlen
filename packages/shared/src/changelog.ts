...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    notes: {
      en: [
        "Home is two columns now: what waits on you on the left, the day on the right. The left column gathers everything open in three groups, drafts to approve, questions to answer and tasks to do, oldest first. The right column is one time axis with a line on the current minute: what already ran above it, what is scheduled next below it, and tomorrow under its own label.",
        "The report is no longer a block on Home. It is the first row of the day and opens full width when you press it, with its lists and charts, a button to run it again and the way back. Every other run opens the same way, so what an automation produced is always one press away.",
        "A report says what changed since the one before it: a message marked new or updated, an item waiting since Monday, and anything you have dealt with struck through once before it goes. An item that needs you stays in the following reports until it is closed, routine mail that has not moved is not shown again, and the assistant names the sections itself instead of filling fixed ones.",
        "The automations page is a list grouped by what starts an automation: a schedule, new mail, or you. Each row carries the first sentence of its instruction and, on the right, whether it is running, failed last time, is paused, or when it runs next. The three-dot menu runs, pins, pauses, edits or deletes it on the spot, and a row opens its settings together with everything it has run.",
        'A schedule is now picked rather than typed, with weekdays, days of the month and months as real controls. An automation whose schedule the picker cannot express keeps it and says "Custom schedule".',
        "A new weekly automation watches for the jobs you keep asking for by hand and files each one as a to-do that proposes the automation for it. Something you dismiss is never proposed again.",
        "Settings has a Profile: your name, a picture, and a few lines about you and how you work. Name and text reach the assistant at the start of every conversation, so it addresses you properly and knows who it is writing for. Both stay on this computer.",
        "Settings moved out of the navigation into a profile menu at the bottom of the rail, and splits into General, Profile, Connections, Permissions and Data and app, each with its own address. The navigation itself drags narrower or collapses and comes back at the width you left it, and Leads is always there, with or without a connected CRM.",
        "The app can be set from the chat: appearance, accent colour, language, timezone, what the quick buttons do and whether Marlen starts with the computer arrive as working controls in the conversation. Sign-ins, permissions and sending rights deliberately keep their own places.",
        "When the assistant needs a service you have not connected yet, it offers the whole connection in the chat and carries on with your request as soon as it stands. What you type there goes straight into local settings and never into the conversation.",
        "The chat takes files. Attach images, PDFs, Word documents, Markdown, text, CSV or HTML with the paperclip or by pasting them into the box, up to five per message.",
        "You can point the assistant at particular mail before you ask: search your accounts from the composer and attach the messages you mean, so the answer is about those and nothing else. What the two of you just worked out can be saved as a skill in one click, and a message you started but did not send waits in its own conversation.",
        'The chat itself was rebuilt: "/" opens your skills and your on-demand automations, selected text in a reply can be quoted into your next message, any answer can be copied or asked again, the model is switchable for a single message, and after an hour away a fresh chat starts on its own. The list beside it now switches between your chats and the automation runs.',
        "Answers show their ground: an overview card that puts text, figures, lists, tables and charts in one place, the memory pages that shaped the answer, and the messages it drew on. The assistant can also search your earlier conversations when you refer to something you discussed before.",
        "A draft opens in a reader of its own instead of an inline row: step through every open draft across your inboxes, edit the text and the recipients in place, read the thread it answers, or hand it to the chat to refine. Draft text keeps simple formatting, bold, links, lists and quotes, written so that even Outlook shows it the way you wrote it.",
        "Mail is searched across every connected account in one pass instead of one mailbox at a time, so a question that spans accounts is answered in one go. HTML mail is cleaned on your own machine and shown in a sealed frame, so a message cannot reach past its own content into the app.",
        "Memories, skills and your documents are one library now, pages in the assistant's wiki that you browse under Knowledge, grouped by what they are and listed by title with how often they were used. A page can be kept in context for good, the library is searchable, and two edits to the same page no longer overwrite each other in silence. What sat in the old places moves over by itself the first time you start this version.",
        "Backup became a full export: one ZIP with the database, chats, drafts, automations, wiki, knowledge files and diagnostic logs. Sign-ins, API keys and connected-account credentials stay out of it. File access became Computer access and says plainly what is on, only the Marlene folder, read and write, or full access, where running commands can read and change files whatever the separate switches say.",
        "Fixes: learning your writing style no longer overwrites or deletes a style page you edited by hand; a confirmation no longer closes as though it had worked when sending or discarding failed; a draft you send or delete in your mail app closes its approval on Home; and a list that fails to load offers a retry instead of looking empty.",
        "The window has a new frame: one white workspace carries the page and the chat side by side, only the navigation stays grey, and a single hairline runs between them. Marlen has a new mark, and the accent colour can now be yellow; an installation that never chose one moves to blue.",
        "Long conversations open faster, and the app redraws far less while an answer streams in.",
      ],
      de: [
        "Die Startseite besteht jetzt aus zwei Spalten: links, was auf Sie wartet, rechts der Tag. Links sammelt sich alles Offene in drei Gruppen, Entwürfe zur Freigabe, Fragen zum Beantworten und Aufgaben, das Älteste zuerst. Rechts läuft eine Zeitachse mit einer Linie auf der aktuellen Minute: darüber, was schon gelaufen ist, darunter, was als Nächstes ansteht, und der morgige Tag unter eigener Beschriftung.",
        "Der Bericht ist kein Block mehr auf der Startseite. Er ist die erste Zeile des Tages und öffnet sich auf Klick in voller Breite, mit seinen Listen und Diagrammen, einer Schaltfläche zum erneuten Erstellen und dem Weg zurück. Jeder andere Lauf öffnet sich genauso, das Ergebnis einer Automatisierung ist also immer einen Klick entfernt.",
        "Ein Bericht sagt, was sich seit dem vorigen geändert hat: eine Nachricht als neu oder aktualisiert markiert, ein Punkt, der seit Montag wartet, und Erledigtes einmal durchgestrichen, bevor es verschwindet. Was Sie braucht, bleibt in den folgenden Berichten stehen, bis es geschlossen ist, Routinepost ohne Bewegung erscheint nicht erneut, und die Abschnitte benennt der Assistent selbst, statt feste Fächer zu füllen.",
        "Die Automatisierungsseite ist eine Liste, gruppiert danach, was eine Automatisierung startet: ein Zeitplan, neue E-Mail oder Sie. Jede Zeile trägt den ersten Satz ihrer Anweisung und rechts, ob sie gerade läuft, zuletzt fehlgeschlagen ist, pausiert, oder wann sie das nächste Mal läuft. Das Drei-Punkte-Menü startet, heftet an, pausiert, bearbeitet oder löscht sie an Ort und Stelle, und eine Zeile öffnet ihre Einstellungen samt allen bisherigen Läufen.",
        "Einen Zeitplan wählen Sie jetzt, statt ihn zu tippen, mit Wochentagen, Monatstagen und Monaten als echten Bedienelementen. Eine Automatisierung mit einem Zeitplan, den die Auswahl nicht abbilden kann, behält ihn und zeigt „Eigener Zeitplan“.",
        "Eine neue wöchentliche Automatisierung achtet darauf, worum Sie immer wieder von Hand bitten, und legt dafür je ein To-do an, das die passende Automatisierung vorschlägt. Was Sie verwerfen, wird nicht erneut vorgeschlagen.",
        "Die Einstellungen haben ein Profil: Ihr Name, ein Bild und ein paar Zeilen über Sie und Ihre Arbeitsweise. Name und Text erreichen den Assistenten zu Beginn jedes Gesprächs, damit er Sie richtig anspricht und weiß, für wen er schreibt. Beides bleibt auf diesem Computer.",
        "Die Einstellungen sind aus der Navigation in ein Profilmenü am unteren Rand gewandert und gliedern sich in Allgemein, Profil, Verbindungen, Berechtigungen und Daten und App, jeweils mit eigener Adresse. Die Navigation selbst lässt sich schmaler ziehen oder einklappen und kehrt in der zuletzt gewählten Breite zurück, und Leads ist immer da, mit oder ohne verbundenes CRM.",
        "Die App lässt sich aus dem Chat einstellen: Darstellung, Akzentfarbe, Sprache, Zeitzone, das Verhalten der Schnellaktionen und der Start mit dem Computer kommen als bedienbare Elemente ins Gespräch. Anmeldungen, Berechtigungen und Senderechte behalten bewusst ihren eigenen Ort.",
        "Fehlt dem Assistenten ein Dienst, den Sie noch nicht verbunden haben, bietet er die ganze Verbindung im Chat an und macht mit Ihrer Anfrage weiter, sobald sie steht. Was Sie dort eingeben, geht direkt in die lokalen Einstellungen und nie in das Gespräch.",
        "Der Chat nimmt Dateien an. Hängen Sie Bilder, PDFs, Word-Dokumente, Markdown, Text, CSV oder HTML über die Büroklammer an oder fügen Sie sie direkt ins Eingabefeld ein, bis zu fünf pro Nachricht.",
        "Sie können den Assistenten vor Ihrer Frage auf bestimmte E-Mails richten: Durchsuchen Sie Ihre Konten direkt im Eingabefeld und hängen Sie die gemeinten Nachrichten an, damit es in der Antwort um genau diese geht. Was Sie gerade zusammen erarbeitet haben, lässt sich mit einem Klick als Skill sichern, und eine begonnene, nicht gesendete Nachricht wartet in ihrem eigenen Gespräch.",
        "Der Chat selbst wurde neu gebaut: „/“ öffnet Ihre Skills und Ihre Automatisierungen auf Abruf, markierter Text in einer Antwort lässt sich in die nächste Nachricht zitieren, jede Antwort lässt sich kopieren oder neu anfordern, das Modell wählen Sie für eine einzelne Nachricht, und nach einer Stunde Pause beginnt von selbst ein neuer Chat. Die Liste daneben wechselt jetzt zwischen Ihren Chats und den Läufen der Automatisierungen.",
        "Antworten zeigen, worauf sie beruhen: eine Übersichtskarte, die Text, Kennzahlen, Listen, Tabellen und Diagramme an einem Ort versammelt, die Wissensseiten, die die Antwort geprägt haben, und die Nachrichten, aus denen sie stammt. Der Assistent kann außerdem Ihre früheren Gespräche durchsuchen, wenn Sie sich auf etwas Besprochenes beziehen.",
        "Ein Entwurf öffnet sich in einem eigenen Lesebereich statt in einer Zeile: Blättern Sie durch alle offenen Entwürfe aller Postfächer, ändern Sie Text und Empfänger an Ort und Stelle, lesen Sie den Verlauf, auf den er antwortet, oder geben Sie ihn zum Überarbeiten an den Chat. Entwurfstext behält einfache Formatierung, fett, Links, Listen und Zitate, so geschrieben, dass selbst Outlook sie so zeigt, wie Sie sie gesetzt haben.",
        "E-Mail wird in einem Durchgang über alle verbundenen Konten durchsucht statt Postfach für Postfach, eine kontenübergreifende Frage ist damit in einem Zug beantwortet. HTML-E-Mail wird auf Ihrem eigenen Rechner gesäubert und in einem abgeschotteten Rahmen angezeigt, damit eine Nachricht nicht über ihren eigenen Inhalt hinaus in die App greifen kann.",
        "Erinnerungen, Skills und Ihre Dokumente sind jetzt eine Bibliothek, Seiten im Wiki des Assistenten, die Sie unter Wissen durchsehen, nach Art gruppiert und mit Titel und Nutzungshäufigkeit gelistet. Eine Seite lässt sich dauerhaft im Kontext halten, die Bibliothek ist durchsuchbar, und zwei Änderungen an derselben Seite überschreiben einander nicht mehr stillschweigend. Was an den alten Orten lag, zieht beim ersten Start dieser Version von selbst um.",
        "Aus der Sicherung wurde ein vollständiger Export: ein ZIP mit Datenbank, Chats, Entwürfen, Automatisierungen, Wiki, Wissensdateien und Diagnoseprotokollen. Anmeldungen, API-Schlüssel und Zugangsdaten verbundener Konten bleiben draußen. Aus dem Dateizugriff wurde der Computerzugriff, und er sagt klar, was gilt: nur der Marlene-Ordner, Lesen und Schreiben, oder voller Zugriff, bei dem ausgeführte Befehle Dateien lesen und ändern können, unabhängig von den einzelnen Schaltern.",
        "Behoben: Das Lernen Ihres Schreibstils überschreibt oder löscht keine Stilseite mehr, die Sie selbst bearbeitet haben; eine Bestätigung schließt sich nicht mehr, als wäre alles gut gegangen, wenn Senden oder Verwerfen fehlschlug; ein Entwurf, den Sie in Ihrem Mailprogramm senden oder löschen, schließt seine Freigabe auf der Startseite; und eine Liste, die nicht laden konnte, bietet einen neuen Versuch an, statt leer auszusehen.",
        "Das Fenster hat einen neuen Rahmen: eine weiße Arbeitsfläche trägt Seite und Chat nebeneinander, nur die Navigation bleibt grau, dazwischen läuft eine einzelne Haarlinie. Marlen hat ein neues Zeichen, und die Akzentfarbe kann jetzt Gelb sein; eine Installation ohne eigene Wahl wechselt auf Blau.",
        "Lange Gespräche öffnen schneller, und die App zeichnet deutlich weniger neu, während eine Antwort einläuft.",
      ],
    },
  },
  {
