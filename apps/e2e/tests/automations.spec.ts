...
test("an automation created in the UI is persisted and can be deleted again", async ({
  ...
  const create = page.getByRole("dialog", { name: t("automations.formTitle") });
  await create.getByLabel(t("automations.name")).fill(NAME);
  await create.getByLabel(t("automations.instruction")).fill("Fasse den Posteingang zusammen.");
  await create.getByRole("button", { name: t("automations.frequency.weekdays") }).click();
  await create.getByLabel(t("automations.time")).fill("09:30");
  await create.getByRole("button", { name: t("automations.create"), exact: true }).click();

  // The row reads the instruction's first sentence under the name and sits
  // in the scheduled group.
  const row = page.getByRole("button", { name: new RegExp(NAME) });
  await expect(row).toBeVisible();
  await expect(row).toContainText("Fasse den Posteingang zusammen.");
  await expect(
    page.getByRole("heading", { name: new RegExp(t("automations.groupSchedule"), "i") }),
  ).toBeVisible();
  ...
  // Pressing the row opens its dialog through the URL.
  await row.click();
  await expect(page).toHaveURL(new RegExp(`[?&]automation=${saved.id}`));
  ...
  await dialog.getByRole("button", { name: t("automations.delete"), exact: true }).click();
  await page
  ...
  await expect(page).not.toHaveURL(/[?&]automation=/);
